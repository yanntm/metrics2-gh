...
public class LCOMHS extends CohesionCalculator {
            ...
            throws InvalidSourceException
    {
        ...
        System.out.println("Setting HS to " + value + " for " + source.getName());
        source.setValue(new Metric(LCOMHS, value));
    }

    /**
     * @return double (avg(m(a)) - m)/(1 - m)
     *  where m(a) is the number of methods that access a
     */
    private double calculateResult(HashMap<IField,HashSet<IMethod>> attributeAccessedByMap)
    {
        int sum = 0;
        int numAttributes = 0;
        Set<IMethod> allMethods = new HashSet<IMethod>();
        // TODO remove redundant calculation
        for (Iterator<HashSet<IMethod>> i = attributeAccessedByMap.values()
                .iterator(); i.hasNext(); numAttributes++)
        {
